        ...
        if prob == 'regression':
            for i in range(dim_y):
                ...
                for j in range(len(lstm)):
                    self.lstm[-1].append(nn.LSTM(self.net[j], self.net[j + 1], batch_first=True))
                self.fc.append(nn.Linear(lstm[-1], 1))
        elif prob == 'classification':
            for i in range(len(lstm)):
                self.lstm.append(nn.LSTM(self.net[i], self.net[i + 1], batch_first=True))
...
class LstmModel(NeuralNetwork):
    ...
    def fit(self, X, y):
        ...
        # Model creation
        self.model = LongShortTermMemory(self.dim_X, self.dim_y, self.args['lstm'], self.args['mode'],
